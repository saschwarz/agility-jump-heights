...
function updateTable(height){var resultsTemplate=document.getElementById('results-template').innerHTML;var resultsRow=document.getElementById('results');if(!height){resultsRow.innerHTML='';resultsRow.className='u-max-full-width';return;}
...
function onHeightChange(){var height=getHeight();updateTable(height);_.each(graphData,function(config){window.setTimeout(function(){updateGraph(config.gnode,config.filtered,config.data.slice(),config.layout,height)});});}
