...
try{var annotated=annotate(config);var gnodes=[];var graphConfigs=['Jump Heights US Organizations<br>"Regular/Championship"','Jump Heights US Organizations<br>"Performance/Preferred"','Jump Heights US Organizations<br>"Veterans/Specialist"','Jump Heights Canadian Organizations','Jump Heights International<br>Championship Organizations','WAO Future vs Current','UKI Future vs Current<br>Select is one jump height lower',];var graphData=_.map(graphConfigs,function(title,i){var index=i+1;var filtered=_.filter(annotated,{group:index});var data=setup(filtered);var gnode=Plotly.d3.select('#graph'+index).node();gnodes.push(gnode);return{gnode:gnode,filtered:filtered,data:data,layout:buildLayout(filtered,title)};})
